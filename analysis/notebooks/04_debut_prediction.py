...
def _(mo):
    ...
    The baselines predict the global or debut-class median terminal subtotal.
    Ridge regression represents a regularized linear model. Gradient boosting
    is the proposed nonlinear model.
...
def _(mo):
    ...
    Each row adds only the features named in its label. The earlier notebook
    accidentally included class variables in the “score only” row; that defect
    is corrected here.
    """)
    return
...
def _(mo):
    ...
    PSA has the largest sample and the largest held-out error. PIA and PIO
    estimates are based on very small samples and should not support strong
    class-specific conclusions.
    """)
    return
...
def _(mo):
    ...
    The largest misses are mostly overpredictions in PSA. This is practically
    important because the project owner considers overrating a group more
    harmful than underrating it. The database cannot explain these cases using
    staff, membership, rehearsal time, repertoire, or show completion.
    """)
    return
